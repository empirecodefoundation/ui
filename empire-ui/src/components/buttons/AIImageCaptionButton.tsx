...
export const AIImageCaptionButton: React.FC<AIImageCaptionButtonProps> = ({
  ...
  return (
    <Card
      className={cn(
        "w-full max-w-lg mx-auto bg-white border-black rounded-xl",
        cardClassName
      )}
    >
      <CardHeader className="pb-0">
        <CardTitle className="text-2xl font-bold flex items-center gap-x-2 justify-between">
          <Input
            ...
            className={cn(
              "w-full border border-gray-300 rounded-3xl p-2 text-sm",
              inputClassName
            )}
          />
          <motion.button
      ...
      <CardContent className="space-y-1">
        ...
        {caption && (
          <div className="w-full text-center">
            <p className="text-sm text-gray-500 border-t-2 border-black pt-1">
              Generated Caption
            </p>
            <Textarea
              value={caption}
              readOnly
              className={cn(
                "w-full bg-gray-50 border h-auto border-gray-300 rounded-xl resize-none py-2 px-3",
                captionClassName
              )}
            />
          </div>
